...
Q3-2.5 对 MMSA、Self-MM、MMSA-FET 与 CMU-MultimodalSDK 的固定 commit/历史代码做 provenance 审计。Attachment4 高度吻合公开 MMSA/Self-MM MOSEI aligned 格式，但没有 source ID、上游 aggregate hash 或生成配置，不能确认为同一来源。Pinned `bert-base-uncased` fast tokenizer 的 ID、mask、segment IDs 对 20/20 exact match，可把 `text_bert` token slot 映射到 `raw_text` offset；但 P2 使用的 768-D `text` row 是否与该 slot 同索引语义没有附件级源码/manifest 证明。A/V 的 COVAREP/FACET 维度和 SDK/FET word alignment 只说明候选方法，原始 ID 与 interval 仍缺。故 token 子映射 VERIFIED，TEXT feature grounding/AUDIO/VISION 均 UNVERIFIED，整体 `BLOCKED_BY_GROUNDING`。没有改 HEAF/P2、无标签/全量推理。实现及证据见 `E2026/src/q3/text_grounding.py`、`E2026/outputs/q3/q3_alignment_provenance_report.md`、`E2026/outputs/q3/provenance_sources.md`。

Q3-2 已先锁定 HEAF 方法（`E2026/configs/final/q3_heaf.yaml`、`E2026/outputs/q3/q3_method_lock.{md,json}`），然后只审计 Attachment4 对齐版。20 pkl 与 20 MP4 的 ID 精确对应，接口合法，但 pkl 没有逐槽文本/音频/视频时间映射；目前局部 grounding 等级 **UNVERIFIED**，结论 `BLOCKED_BY_GROUNDING`。不允许据视频时长均分 50 槽或照搬 Q1 窗口；未运行全量解释或生成最终 CSV。证据及两项候选路线见 `E2026/outputs/q3/q3_grounding_audit.{md,json}`。
