...
正文现包含 Q1、Q2、Q3、模型评价和结论。Q1 总览图、模态时间对齐示意图与样本追踪图均在正文；原有两张 Q1 消融图保留在附录。Q2 结构图与缺失比例曲线已替换原占位，模态/位置热图继续使用已有图件；Q2 三次初始化与误差分析图仍为待替换占位。题目、摘要、关键词尚待最终定稿。未改写 Q1 的冻结表格数据。
