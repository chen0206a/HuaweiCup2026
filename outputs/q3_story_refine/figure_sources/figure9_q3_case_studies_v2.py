...
BASE = Path('D:\\华为杯\\outputs\\q3_story_refine\\figures\\q3\\fig16_q3_case_explanations')
...
CLASS_LABELS = {"Negative": "消极", "Neutral": "中性", "Positive": "积极"}
...
def add_prediction(fig, card: dict, x: float, y: float) -> None:
    ...
    fig.text(x, y - 0.025, f"置信度  {pred['confidence']:.3f}     分类主导模态  {MODALITY_LABELS[primary]}",
             fontsize=7.4, ha="left", va="top", color=BLACK)
...
def add_shapley(fig, card: dict, bounds) -> None:
    ...
    ax.set_xlabel("分类对数优势贡献", fontsize=7.0, labelpad=1.0)
...
def add_temporal(fig, card: dict, bounds) -> None:
    ...
    ax.set_ylabel("对数优势下降", fontsize=7.0, labelpad=0.5)
...
def draw(cards: dict[str, dict], frames: dict[str, list[Path]], fragment: str):
    ...
    fig.text(0.088, 0.487, "视觉主导典型案例（样本02）", fontsize=9.0,
             fontweight="bold", ha="left", va="top", color=BLACK)
    ...
    fig.text(0.706, 0.922, "局部遮挡曲线", fontsize=8.0,
             fontweight="semibold", ha="left", va="bottom", color=BLACK)
    ...
    fig.text(0.061, 0.585, "原文字符级", fontsize=7.6,
    ...
    fig.text(0.706, 0.443, "局部遮挡曲线", fontsize=8.0,
             fontweight="semibold", ha="left", va="bottom", color=BLACK)
