...
def main():
    ...
    ax_a.text(
        0.98, 0.97,
        f"{int((delta > 0).sum())}/3 个种子提升\n平均增量 = {delta.mean():+.4f} ± {delta.std(ddof=1):.4f}",
        transform=ax_a.transAxes, ha="right", va="top", fontsize=6.7, color=DARK,
        bbox={"boxstyle": "round,pad=0.25", "fc": "white", "ec": GRID, "lw": 0.55},
    )
